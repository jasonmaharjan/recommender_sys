...
def _load_state() -> State:
    ...
    all_splits = pd.concat(
        [
            pd.read_parquet(PROCESSED / f"{s}.parquet", columns=["user_id", "user_idx", "movie_id", "item_idx"])
    ...
    user_id_to_idx = dict(zip(users_unique["user_id"].astype(int), users_unique["user_idx"].astype(int), strict=True))
    item_idx_to_movie_id = dict(zip(items_unique["item_idx"].astype(int), items_unique["movie_id"].astype(int), strict=True))
    ...
    return State(
        model=model, index=index, device=device,
...
async def recommend(user_id: int, k: int = DEFAULT_K) -> RecommendResponse:
    ...
    payload = {"user_id": user_id, "k": k, "recommendations": [r.model_dump() for r in recs], "latency_ms": latency_ms}
    if state.redis is not None:
        await state.redis.setex(cache_key, CACHE_TTL_SECONDS, json.dumps(payload))  # type: ignore[attr-defined]
